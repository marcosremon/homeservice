...
class AlexaRequestVerifier(IAlexaRequestVerifier):
    ...
    async def amazon_approve(self, request: Request) -> bool:
        ...
        try:
            ...
            return await self.verify(signature_cert_chain_url, signature, raw_body)
    ...
    async def verify(self, signature_cert_chain_url: str, signature: str, raw_body: bytes) -> bool:
        ...
        try:
            ...
            if not signature_cert_chain_url or not signature:
                return False

            # 2. Validar que la URL del certificado cumple los requisitos de Amazon
            if not self._is_valid_certificate_url(signature_cert_chain_url):
                return False

            # 3 y 4. Obtener (o cachear) el certificado, validando cadena, vigencia y SAN
            certificate = await self._get_certificate(signature_cert_chain_url)
            if certificate is None:
                return False

            # 5. Verificar la firma criptografica del cuerpo
            if not self._verify_signature(certificate, raw_body, signature):
                return False

            # 6. Verificar timestamp (evita replay attacks)
            if not self._is_timestamp_valid(raw_body):
                return False
    ...
    @staticmethod
    def _is_valid_certificate_url(url: str) -> bool:
        ...
        normalized_path = _normalize_path(uri.path)
        if not normalized_path.lower().startswith(_AMAZON_CERT_PATH):
            return False
    ...
    async def _get_certificate(self, cert_url: str) -> x509.Certificate | None:
        ...
        async with _cache_lock:
            ...
            response = await self._http_client.get(cert_url)
            ...
            try:
                ...
                _logger.warning("AlexaRequestVerifier -> cadena de certificado no valida: %s -> %s", cert_url, ex)
                return None

            _cert_cache[cert_url] = leaf
            return leaf
    # endregion

    # region _is_still_valid
    @staticmethod
    def _is_still_valid(certificate: x509.Certificate) -> bool:
    ...
    @staticmethod
    def _verify_signature(certificate: x509.Certificate, raw_body: bytes, base64_signature: str) -> bool:
        ...
        try:
            ...
            signature_bytes = base64.b64decode(base64_signature, validate=True)

            # verify lanza InvalidSignature si no cuadra; no devuelve bool.
            public_key.verify(signature_bytes, raw_body, padding.PKCS1v15(), hashes.SHA1())
            return True
        except Exception:
            return False
    # endregion
    ...
    @staticmethod
    def _is_timestamp_valid(raw_body: bytes) -> bool:
