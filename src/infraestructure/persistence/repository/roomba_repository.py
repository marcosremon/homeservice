    ...
    async def create_roomba(self, create_roomba_request: CreateRoombaRequest) -> CreateRoombaResponse:
        ...
        try:
            ...
            if any_roomba is not None:
                ...
                roomba: Roomba | None = await self._session.scalar(select(Roomba).where(Roomba.device_id == device.device_id))
                if roomba is not None:
                    create_roomba_response.response_code = ResponseCodes.ANY_ROOMBA_EXIST
                    create_roomba_response.is_success = True
                    create_roomba_response.message = "roomba exist"
    ...
    async def patch_roomba_state(self, patch_roomba_state_request: PatchRoombaStateRequest) -> PatchRoombaStateResponse:
        ...
        try:
            ...
            if roomba is None:
                ...
                # rechaza el UPDATE.
                event_time: datetime = patch_roomba_state_request.event_time
                ...
                if patch_roomba_state_request.battery_percent > 0:
                    roomba.battery_percent = patch_roomba_state_request.battery_percent

                if not GeneralUtils.is_null_or_empty(patch_roomba_state_request.pmap_id):
                    roomba.pmap_id = patch_roomba_state_request.pmap_id

                if not GeneralUtils.is_null_or_empty(patch_roomba_state_request.user_pmapv_id):
                ...
                patch_roomba_state_response.message = f"Roomba state updated successfully (phase {roomba.phase}, battery {roomba.battery_percent}%)."
        except Exception as ex:
            await self._session.rollback()
